WITH trips AS (
  SELECT
    taxi_id,
    trip_start_datetime,
    DATETIME_ADD(trip_start_datetime, INTERVAL trip_seconds SECOND) AS trip_end_datetime_exact,
    trip_seconds
...
events AS (
  ...
  UNION ALL
  SELECT taxi_id, trip_end_datetime_exact AS event_datetime, -1 AS delta FROM trips
),
event_points AS (
...
active_intervals AS (
  SELECT
    ...
    DATETIME_DIFF(next_event_datetime, event_datetime, SECOND) / 60.0 AS mins
  FROM coverage
  WHERE active_trip_count > 0
    AND next_event_datetime IS NOT NULL
    AND next_event_datetime > event_datetime
)
SELECT 'RAW_TRIP' as type, trip_start_datetime as start_time, trip_end_datetime_exact as end_time, trip_seconds / 60.0 as mins
FROM trips
UNION ALL
